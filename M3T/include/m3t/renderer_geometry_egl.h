...
#ifndef M3T_INCLUDE_M3T_RENDERER_GEOMETRY_EGL_H_
#define M3T_INCLUDE_M3T_RENDERER_GEOMETRY_EGL_H_

#include <glad/glad.h>
#include <EGL/egl.h>
// X11.h defines many generic macros polluting the global namespace
#ifdef Success
  #undef Success
#endif
#ifdef Status
 ...
 * provides this information together with an *EGL* context required for
 * rendering.
 *
 * \details Rendering data with *VAOs* and *VBOs* for all assigned
 * bodies is stored in `render_data_bodies`. The *EGL* context can be handled
 ...
 * *EGL* thread-safety requirements. Calling the `SetUp()` method initializes
 * *EGL*, creates an *EGL* context, and creates *VAOs* and *VBOs*. \ref Body
...
class RendererGeometry {
 private:
  // Count the number of instances to manage the EGL library
...
#endif  // M3T_INCLUDE_M3T_RENDERER_GEOMETRY_EGL_H_